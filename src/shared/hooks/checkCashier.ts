...
import { createWorker, getEmployeesByName } from "src/shared/schema"
/* eslint-disable */
export const useCheckCashier = async (name: string) => {
    ...
    const parseResponse = useParser(request.data).RK7QueryResult.CommandResult.RK7Reference

    if (parseResponse.Items.length <= 0) {
        const newCashierSchema = createWorker(name, parentIdent)
        const { error, data, isAxiosError, code } = await sendData(newCashierSchema)
        return {
            error,
            data: useParser(data),
            isAxiosError,
